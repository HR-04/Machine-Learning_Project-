...
def load_saved_artifacts():
    ...
    # Combine the path with the 'artifacts' folder inside the 'server' directory
    artifacts_dir = os.path.join(script_dir, 'artifacts')

    # Open class_dictionary.json using the absolute path
    with open(os.path.join(artifacts_dir, "class_dictionary.json"), "r") as f:
        __class_name_to_number = json.load(f)
        __class_number_to_name = {v: k for k, v in __class_name_to_number.items()}

    # Open saved_model.pkl using the absolute path
    global __model
    if __model is None:
